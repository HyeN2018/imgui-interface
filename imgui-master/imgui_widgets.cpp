...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImGui::InvisibleButton(label, ImVec2(width + 80, height + stroke));
    ...
    ImU32 col_bg;
    ImU32 col_bg2;
    ImU32 col_stroke = ImGui::GetColorU32(ImLerp(ImColor(25, 25, 27), ImVec4(0.098f, 0.098f, 0.106f, 1.f), t));

    col_bg = ImGui::GetColorU32(ImLerp(ImColor(38, 38, 41), ImVec4(0.098f, 0.098f, 0.106f, 1.f), t));
    col_bg2 = ImGui::GetColorU32(ImLerp(ImColor(70, 70, 70, 255), ImVec4(0.933f, 0.631f, 0.055f, 1.f), t));

    draw_list->AddRectFilled(ImVec2(p.x, p.y), ImVec2(p.x + 55, p.y + 28), col_stroke, height * 0.5f);
    draw_list->AddRectFilled(ImVec2(p.x + stroke, p.y + stroke), ImVec2(p.x + 55 - stroke, p.y + 28 - stroke), col_bg, height * 0.5f);
    draw_list->AddCircleFilled(ImVec2(p.x + radius + stroke + t * (width - radius * 2.0f), p.y + radius + stroke/2), radius - stroke*2.0f, col_bg2);

    if (label_size.x > 0.0f)
    {
        ImGui::PushFont(fontidk);
        ImGui::PushStyleColor(ImGuiCol_Text, ImVec4(1.f, 1.f, 1.f, 1.f));
        RenderText(ImVec2((p.x + width) + 15, (p.y + height/4) - stroke), label);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    // Render shape
    float t = 2.f;
    ImU32 stroke_col = GetColorU32(ImGuiCol_Border);
    ...
    {
        window->DrawList->AddRectFilled(bb.Min, ImVec2(value_x2 + 5, bb.Max.y + 5), stroke_col, style.FrameRounding, NULL);
        window->DrawList->AddRectFilled(ImVec2(bb.Min.x + stroke, bb.Min.y + stroke), ImVec2(value_x2 + 5 - stroke, bb.Max.y + 5 - stroke), frame_col, style.FrameRounding, (flags & ImGuiComboFlags_NoArrowButton));
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        ImU32 arrow_col = GetColorU32(ImGuiCol_CheckMark);
        //window->DrawList->AddRectFilled(ImVec2(value_x2, bb.Min.y), bb.Max, bg_col, style.FrameRounding, (w <= arrow_size) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersRight);
        RenderArrow(window->DrawList, ImVec2(value_x2 - 25, bb.Min.y + style.FramePadding.y + 2.5f), arrow_col, ImGuiDir_Down, 1.0f);
    }
    ...
    if (preview_value != NULL && !(flags & ImGuiComboFlags_NoPreview))
    {
        if (g.LogEnabled)
            LogSetNextTextDecoration("{", "}");
        RenderTextClipped(ImVec2(bb.Min.x + stroke + 10, bb.Min.y + 5), ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);
    }
    if (label_size.x > 0)
        RenderText(ImVec2(bb.Max.x + style.ItemInnerSpacing.x + stroke + 10, bb.Min.y + style.FramePadding.y + 2.5f), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // Draw frame
    const ImU32 stroke_col = GetColorU32(ImGuiCol_Border);
    const ImU32 frame_col = GetColorU32(g.ActiveId == id ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg);
    RenderNavHighlight(frame_bb, id);
    RenderFrame(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(frame_bb.Max.x +5, frame_bb.Max.y + 5), stroke_col, true, g.Style.FrameRounding);
    RenderFrame(ImVec2(frame_bb.Min.x + stroke, frame_bb.Min.y + stroke), ImVec2(frame_bb.Max.x + 5 - stroke, frame_bb.Max.y + 5 - stroke), frame_col, true, g.Style.FrameRounding);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x)
        window->DrawList->AddRectFilled(ImVec2(grab_bb.Min.x + 1, grab_bb.Min.y + 1), ImVec2(grab_bb.Max.x + 4, grab_bb.Max.y + 4), GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
    ...
    if (g.LogEnabled)
        LogSetNextTextDecoration("{", "}");
    RenderTextClipped(ImVec2(frame_bb.Max.x - stroke - 40, frame_bb.Min.y + stroke), ImVec2(frame_bb.Max.x + stroke, frame_bb.Max.y + stroke), value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));

    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x + 10, frame_bb.Min.y + style.FramePadding.y), label);
